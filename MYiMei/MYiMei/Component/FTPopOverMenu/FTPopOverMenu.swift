...
public class FTPopOverMenu : NSObject {
    ...
    fileprivate lazy var backgroundView : UIView = {
        ...
        if self.configuration.globalNotContainBarShadow {
            let barView = UIView(frame: CGRect(x: 0, y: 0, width: UIScreen.ft_width(), height: self.configuration.navigationBarHeight))
            let bgView = UIView(frame:  CGRect(x: 0, y: self.configuration.navigationBarHeight, width: UIScreen.ft_width(), height: UIScreen.ft_height() - self.configuration.navigationBarHeight))
            bgView.backgroundColor = UIColor.black.withAlphaComponent(self.configuration.shadowAlpha)
